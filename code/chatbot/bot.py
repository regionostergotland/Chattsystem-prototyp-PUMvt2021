...
The chatbot uses a database which contains a number of responses
to different user inputs, this includes questions,
phrases and informative answers.

The bot uses an algorithm that matches the user input
against the database and tries to get the most similar
response possible from the given inputs in the database.
"""
import inspect
import sys
import os
...
def bot_response(user_input, list_of_matches):
    ...
    elif response_flag == 0 or bot_response is False:
...
def search(input):
    """
    This function searched in the database for user inputs that match the
    current user input. It sends a list of matching responses from the
    database to the bot_response -function and returns the answer from
    that function.
    """
    ...
    for word in input.split():
        ...
        if match != []:
            for question in match:
                if question not in all_matches:
                    # Only the questions are relevant
                    # and they are put in the list
...
def bot_main(input):
    """
    This is the function that is used by switchboard.py to contact the bot.
    If there is an exit message in the input, then the bot sends a exit
    message back. If there is a greeting (for example 'hej') or another
    kind of message then it sends back a response from the database.
    """
    sign_list = ['?', ',', '.', '!', '(', ')', '"', '-', '_', '*', '+']
    input = input.lower()
    for char in input:
    ...
    if input in exit_list:
        ...
        if greeting_response(input) is not None:
